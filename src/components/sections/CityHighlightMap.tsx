  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      /* Esri Light Gray Canvas (base + labels), no API key required — see
         ServiceArea.tsx for why this replaced the old CartoDB tile URL. */
      L.tileLayer(
      ...
      L.marker([lat, lon], { icon: pin })
        ...
        .bindPopup(
          `<div style="font-family:Inter,sans-serif;font-weight:700;color:#1E3A6E;font-size:13px;line-height:1.4">
            ${name}, WA<br>
            <span style="font-weight:400;color:#555;font-size:12px">All Phase Plumbing · (206) 309-1088</span>
          </div>`,
          { maxWidth: 220 },
        )
        .openPopup();
